...
- A submission and, for Agent runs, one schema-validated evidence-ledger sidecar are copied in one direction by the WSL host only after the agent Compose project has stopped.
- The evidence-ledger sidecar contains only example identifiers, frozen-seed paragraph IDs, and final-ledger paragraph IDs. It contains no text, labels, predictions, Gold, feedback, prompts, traces, or general state. Its SHA256 and schema version are bound into the sealed submission manifest.
...
- Sealed submissions still contain exactly `predictions.jsonl`, `manifest.json`, and `SHA256SUMS`; no trace, prompt, state, secret, source, gold, or feedback. The separately transferred, hash-bound evidence-ledger sidecar is the only permitted auxiliary per-example artifact.
